...
public class PostMemoryAdapter: PostAdapter {
    var items: [String:Post] = [:]

    public func findAll(onCompletion: @escaping ([Post], Error?) -> Void) {
        onCompletion(items.map { $1 }, nil)
    }

    public func create(_ model: Post, onCompletion: @escaping (Post?, Error?) -> Void) {
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (Post?, Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: Post, onCompletion: @escaping (Post?, Error?) -> Void) {
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (Post?, Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
